...
function GemisExperience({ slide, onSlideChange }: { slide: number; onSlideChange: (slide: number) => void }) {
  ...
  const slides = [
    <div className="gemis-intro" key="intro"><div className="gemis-intro__copy"><span className="eyebrow">Grupo GEMIS · referente de investigación y extensión</span><img src="/media/gemis/logo-gemis.png" alt="Grupo GEMIS" /><h4>{gemis.intro.title}</h4><p>{gemis.intro.description}</p></div><aside className="gemis-qr"><img src="/media/gemis/qr-gemis.png" alt="Código QR para visitar Grupo GEMIS" /><strong>Conocé más sobre GEMIS</strong><small>Escaneá para visitar expoisi.com.ar/Gemis/index</small></aside></div>,
